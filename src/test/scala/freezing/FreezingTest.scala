...
class FreezingTest extends AnyFreeSpec with Matchers:
  "Assignment" - {
    "should optimise" in {
      ...
      given ZipCodes   = Map("20500" -> ZipCode("20500", 38.8946, -77.0355))
      given Polarities = (Nil, Nil)
    ...
    "should optimise with locality" in {
      ...
      given ZipCodes   =
        Map(
          "20500" -> ZipCode("20500", 38.8946, -77.0355),
  ...
  "Freezing Teams" - {
    ...
    "should preclude antagonists" in {
      (List(Set(1L, 2L), Set(1L, 3L)), Nil).precludes(Set(2L, 3L)) shouldBe false
      (List(Set(1L, 2L), Set(1L, 3L)), Nil).precludes(Set(1L, 2L)) shouldBe true
    }
  }
end FreezingTest
